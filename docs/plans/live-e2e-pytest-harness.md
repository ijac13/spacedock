...
- **Root cause:** The helper's opt-in design was CI-uninvolved by construction. The gate checked for a file that exists only on operator workstations; CI took the `None` fallback path through `_clean_env()` every time, reusing `HOME=/home/runner` across every pytest-xdist worker. Concurrent cache writes from parallel `claude` child processes then produced the correlated inner-check failures.
- **Fix:** Restructure `_isolated_claude_env()` as a three-branch decision tree. (a) When `~/.claude/benchmark-token` is present and non-empty, behave as before — fresh HOME tmpdir, inject `CLAUDE_CODE_OAUTH_TOKEN`, drop `ANTHROPIC_API_KEY`. (b) When the token file is absent but `ANTHROPIC_API_KEY` is set, allocate a fresh HOME tmpdir and pass the API key through. This is the CI path. (c) When neither credential is available, return `None` so the caller falls back to `_clean_env()` without false isolation claims. Docstring rewritten to describe the decision tree and the parallel-pytest-xdist rationale.
- **Files changed:**
  - `scripts/test_lib.py` — `_isolated_claude_env` rewritten with three-branch decision tree; name retained (isolation is unconditional when auth exists).
  - `tests/test_test_lib_helpers.py` — imported `_isolated_claude_env`; added three focused unit tests (`test_isolated_claude_env_injects_oauth_token_when_token_file_present`, `test_isolated_claude_env_preserves_api_key_when_no_token_file`, `test_isolated_claude_env_returns_none_when_no_auth_available`) using `monkeypatch` + `tmp_path`, each exercising exactly one branch.
- **Static result:** `308 passed, 21 deselected, 10 subtests passed in 6.76s`. Targeted `tests/test_test_lib_helpers.py -v` reports `13 passed in 0.02s`, with the three new branch tests named above all green.
...
## Stage Report — Cycle 7 (2026-04-15) — skipif-no-tty on test_interactive_poc_live pending #155

- **Goal:** Unblock PR #94 merge by gating `test_interactive_poc_live` on `sys.stdin.isatty()` so it still runs for developers on a real terminal but skips cleanly on headless CI runners. PR #94 cycle-6 CI showed 4/5 jobs green and three claude jobs red with identical `AssertionError: turn 1 did not echo ALPHA_MARKER` — a PTY/TTY headless-runner issue distinct from the assertion drift handled by #154.
- **Direction change during validation:** Initial implementation used an unconditional `@pytest.mark.skip(...)` (committed as `ab88f99e`). Team-lead redirected to `@pytest.mark.skipif(not sys.stdin.isatty(), ...)` so local runs keep coverage. This cycle-7b commit converts `skip` to `skipif` on top of `ab88f99e`. Reason string and grep anchor (`#155`) preserved verbatim.
- **Root cause reference:** Task #155 (just filed on main) — the `InteractiveSession` PTY smoke requires a real controlling terminal which GitHub's `ubuntu-latest` runner does not provide. `sys.stdin.isatty()` returns `False` in headless CI and `True` under a developer TTY, so the predicate is runner-agnostic and CI-provider-agnostic.
- **Files changed (1):**
  - `tests/test_interactive_poc.py` — replaced the unconditional skip with `@pytest.mark.skipif(not sys.stdin.isatty(), reason="requires real TTY; CI runners are headless — see #155")` as the outermost decorator on `test_interactive_poc_live`; preserved existing `live_claude` and `serial` markers; `test_interactive_poc_offline` untouched. `sys` was already imported at module top.
- **Scope discipline:** Single-file edit, replacing the `@pytest.mark.skip(...)` line with a three-line `@pytest.mark.skipif(...)` block. No changes to `test_interactive_poc_offline`, conftest, Makefile, CI workflow, or any other test. The reason string uses EXACTLY `"requires real TTY; CI runners are headless — see #155"` so a future grep for `#155` locates this decorator.
- **Static result:** `311 passed, 21 deselected, 10 subtests passed in 12.58s`. skipif is invisible to static because `live_claude` already deselects the live-only test.
- **Forward pointer:** When #155 lands (e.g., via an alternative transport for the PTY smoke or a self-hosted TTY-capable runner), this decorator is removed. One grep of `#155` across the tree finds it.
- **Recommendation:** PASSED — narrow cycle-7 deliverable is the skipif predicate plus this stage-report update; two commits (`ab88f99e` skip + follow-up skipif) both on-branch.

## Stage Report — Cycle 8 (2026-04-15) — no-op (pre-empted by cycle 7b)

- **Dispatch intent:** Convert `test_interactive_poc_live` from unconditional `@pytest.mark.skip` to `@pytest.mark.skipif(not sys.stdin.isatty(), reason="requires real TTY; CI runners are headless — see #155")` so developers on a real terminal still get PTY coverage while headless CI skips cleanly.
- **Why this is a no-op:** The cycle 7 validation received a mid-cycle redirect from the team-lead with the same `skip` → `skipif(not sys.stdin.isatty())` ask. Cycle 7b committed that swap on-branch as `2ffbc235` (`tests: #148 cycle 7 — skipif-no-tty on test_interactive_poc_live pending #155`) and pushed it. By the time cycle 8 was dispatched, HEAD already matched the target: `tests/test_interactive_poc.py:43-46` holds the `@pytest.mark.skipif(not sys.stdin.isatty(), ...)` decorator with `import sys` already on line 6 and the `live_claude` + `serial` markers preserved.
- **Evidence of the pre-existing target state (captured pre cycle-8 commit):** `make test-static` → `311 passed, 21 deselected, 10 subtests passed in 11.19s`. `uv run pytest tests/test_interactive_poc.py::test_interactive_poc_live -rs -v` → `SKIPPED [1] tests/test_interactive_poc.py:43: requires real TTY; CI runners are headless — see #155` — predicate fires under headless stdin and the reason string is byte-for-byte correct, preserving the `#155` grep anchor.
- **Scope of cycle 8:** Ledger-only. No code or test changes, no decorator edits, no test-selection changes. The delivering commit remains `2ffbc235`; this cycle only adds the present report section so the dispatch → report linkage is recoverable by future `grep` across the entity body.
- **Forward pointer:** When task #155 resolves (e.g., self-hosted TTY-capable runner, or an alternative transport that obviates the PTY requirement), the skipif decorator is removed. One grep of `#155` across the tree still finds the single touchpoint.
- **Recommendation:** PASSED — cycle-8 deliverable is this ledger entry; the semantic change it asked for is already live at `2ffbc235` on the branch.

## Stage Report — Cycle 9 (2026-04-15) — second PTY-sibling skipif on test_single_entity_mode

- **Goal:** Extend the same `skipif(not sys.stdin.isatty(), ...)` guard to `tests/test_single_entity_mode.py::test_single_entity_mode`, a second live PTY test that cycles 7/8 did not cover. PR #94 run 24438136431 on commit `6c71daae` showed `claude-live-bare` failing with `FAIL: FO booted and acknowledged workflow` (0 passed / 1 failed) — same headless-TTY failure class as `test_interactive_poc_live`, same `InteractiveSession` spawn path.
- **Root cause reference:** Task #155 — `InteractiveSession` PTY tests require a real controlling terminal that `ubuntu-latest` runners do not provide. Cycles 7/8 only gated `test_interactive_poc_live`; cycle 9 discovers `test_single_entity_mode` is its PTY sibling (not to be confused with `test_single_entity_team_skip`, which exercises the non-PTY team-skip path). A single grep of `#155` now returns two touchpoints; both are removed together when #155 is resolved.
