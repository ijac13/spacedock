...
2. **Empirical probe (AC-2 load-bearing)** — DONE. Constructed `/tmp/probe-162/test-mod.md` with three flavors of `##` content inside the `## Agent Prompt` body: (a) nested headings inside a fenced markdown block, (b) `##` inline as emphasis-only text outside fences, (c) `##` inside a Python string literal in a fenced code block. Ran `awk '/^## Agent Prompt$/{flag=1; next} flag' /tmp/probe-162/test-mod.md` — output preserved all three flavors verbatim. **Finding:** the last-section convention is robust against nested `##` by design, because the awk regex `^## Agent Prompt$` is line-anchored and only matches that exact heading; nothing else in the file can trigger section termination because there IS no termination — the convention reads to EOF. The original AC-2 framing ("must NOT be treated as a section terminator") was testing a non-risk. **Action taken:** rewrote AC-2 in place to test the real residual risk — convention violation via trailing `## ` headings AFTER the prompt section — with a fail-loud requirement (helper exits non-zero, stderr names the offending heading). Split out the nested-`##`-accepted case as a separate AC-7 so both halves of the convention have explicit test coverage. Renumbered the live E2E test from AC-11 to AC-12.

3. **Plumbing spot-check** — DONE.
   - `parse_stages_block` (status:97-198) — confirmed it parses README frontmatter `stages:` blocks (a nested `defaults:` + `states:` list schema). Standing-mod parsing is a different schema (flat top-level keys on a mod file). Conclusion: a sibling `parse_mod_metadata(filepath)` is the right move; folding into `parse_stages_block` would conflate two schemas. Reflected in Open Question 2.
   - `cmd_build` and the argparse dispatch (claude-team:76, 515-533) — confirmed verb-noun subcommand shape with two existing siblings (`build`, `context-budget`). Adding `spawn-standing` is analogous-and-additive; no refactor needed. Reflected in Proposed Approach step 2.
   - `lookup_model` (claude-team:417) — confirmed it globs `~/.claude/teams/*/config.json` (ALL teams) and returns the first matching member's model. Wrong scope for the standing-spawn check, which needs team-scoped exactness so two parallel teams can each spawn their own `comm-officer`. **Action taken:** added Open Question 4 and resolved it — propose a sibling `member_exists(team, name) -> bool` scoped to one team's config.json. Updated Proposed Approach step 2 to reference `member_exists` rather than `lookup_model`.

4. **Claude adapter anchor spot-check** — DONE. `claude-first-officer-runtime.md` headings: `## Team Creation` (line 5), `## Worker Resolution` (32), `## Dispatch Adapter` (42). A new `### Standing teammate spawn pass` subsection lands cleanly inside `## Team Creation` after the recovery-procedure prose (after line ~28, single-entity-mode paragraph) and before `## Worker Resolution`. No refactor needed; additive insertion only.

5. **Shared-core anchor spot-check** — DONE. `first-officer-shared-core.md` headings include `## Mod Hook Convention` (line 194) and `## Clarification and Communication` (215). A new top-level `## Standing Teammates` section fits cleanly between them without reorganizing existing material. The `## Dispatch` section (53) accommodates an additive routing bullet for the prose-polisher routing without refactor; the existing event-loop prose stays intact.
...
11. **comm-officer routing** — ATTEMPTED. Sent the stage report's narrative paragraphs to `comm-officer` via SendMessage at 2026-04-16T05:26:21Z (text-passthrough mode, light-touch instruction). Best-effort, non-blocking. If polished prose arrived before commit, it was folded in; if not, this report ships unpolished per the 2-minute fallback rule. Note: the polish request was sent AFTER all substantive ideation work was complete; nothing about correctness depends on the polish.

12. **Completion signal** — pending (last action of this turn, see SendMessage to team-lead below).

**Assessment: plan is gate-ready.** Twelve ACs, each with a named verifier; test plan totals ~21 static + 1 live (comparable to #157 and #159); pilot mod (`docs/plans/_mods/comm-officer.md`) already shipped and validated through two rounds; all four anchor sites confirmed clean for additive insertion; no scope expansion. The probe surfaced one substantive AC re-framing (AC-2 split into AC-2 + AC-7) that strengthens the test plan against the real risk rather than the imagined one.
