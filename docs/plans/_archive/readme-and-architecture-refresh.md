...
completed: 2026-04-10T20:17:25Z
verdict: PASSED
score:
worktree: 
issue:
...
**Cycle 2 — cycle-2 impl ensign died at context limit.** The kept-alive 116 implementation ensign ran out of context mid-cycle-2 and died silently with +43/-53 lines of uncommitted README.md changes in the worktree. FO fresh-dispatched a successor named `spacedock-ensign-readme-and-architecture-refresh-implementation-cycle3` to recover and continue. This observation was filed as task 121 `fo-context-aware-reuse` — the FO should pre-emptively fresh-dispatch when a kept-alive ensign's resident tokens exceed 60% of its context window.

**Cycle 2 addendum (sent mid-cycle-3 to the successor, 2026-04-10 ~18:55Z)** — Two independent FO agents in different downstream projects (a GTM pipeline and an experimentation harness) reviewed the cycle-2 shipped README and surfaced a convergent set of findings. The captain accepted a filtered subset of their feedback:

1. **Factual: "on-demand sandboxing" framing.** Both reviewers independently flag this as incorrect — there is no runtime FO decision-making about isolation. Worktree behavior is declared per stage in the workflow README (`worktree: true|false` under `stages.defaults` or per-state); the FO enforces the declaration. Rephrase to: "Stages declare `worktree: true` when they need isolation; the first officer enforces the declaration" or "You declare per-stage whether work needs isolation, and the first officer enforces it". The claim in the bullet should match the explanation in the following paragraph (which is already correct).
2. **Factual: "scannable" / "in seconds" oversells.** Reports are dense and structured but not scannable in the UI-glance sense, and careful review takes minutes not seconds on large entity files. Soften to "structured evidence for approval, faster than hunting through a sprawling PR" or similar. Drop "scannable" and "in seconds".
3. **Factual: "never lose main to a botched experiment".** True for worktree stages, but not for non-worktree stages — a misbehaving worker on `worktree: false` can still commit to main (soft guarantee via FO Write Scope + convention, not filesystem). Either drop or qualify ("for worktree stages").
4. **Concepts restructure — separate Concept from Roles.** The cycle-2 simplification dropped vocabulary rows (Mission / Work item / Workflow / Stage / Gate) the downstream reviewer says are essential for thinking in the domain. Captain approved restoring them as **a separate Concepts table** alongside the existing **Roles table**. Keep each minimal. The Roles table stays Captain / First Officer / Ensign (3 rows, no Mission). The Concepts table covers the vocabulary a user needs to commission and operate — Mission, Work item, Workflow, Stage, Gate — roughly 5 rows.
5. **Feedback cycles mention — value framing, not implementation.** Add one sentence to How It Works but talk about the value, not the mechanism. Not "feedback-to: {stage} in YAML, ### Feedback Cycles tracking, 3-cycle escalation". Instead something like: "Rejected work automatically bounces back for revision, with a hard cap so you're never stuck in an infinite loop." Or "Reviewers can route rejections straight back into a fresh round of the earlier stage — you don't have to manually wire fixes."
6. **Composability / multi-workflow repos — restore as a mention.** Real, used feature. Proposed: "Multi-workflow repos — spacedock can manage several workflows side by side in the same repo, with entities in one workflow referencing entities in another." One line in What's Different, Use Cases, or equivalent.
7. **Mods / hooks — brief mention.** Real feature used by this workflow (pr-merge mod) and reviewer A's silence-watcher mod. One sentence in How It Works or What Gets Generated naming `_mods/*.md` with `## Hook: startup|idle|merge` as the shape.
8. **NOT ADDRESSED this cycle:** status tool as writer, single-entity / `claude -p` mode. Captain ruled these out for README scope.

Cycle-3 implementer should fold these items into the in-flight cycle-3 work (applying the original 15 cycle-2 directives from above). This addendum is additive, not a re-do.
