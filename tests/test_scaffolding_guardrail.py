...
@pytest.mark.skip(reason="FO violates issue-filing guardrail on haiku — file follow-up task to re-enable")
def test_scaffolding_guardrail(test_project, model, effort):
    """FO refuses to edit scaffolding files and refuses gh issue create without captain approval."""
    t = test_project
    ...
    assert "scaffolding" in fo_text.lower(), "Scaffolding guardrail text missing from assembled agent"

    t.check("assembled first-officer contains issue filing guardrail",
            "issue" in fo_text.lower() and "approval" in fo_text.lower())
    assert "approval" in fo_text.lower(), "Issue filing guardrail text missing from assembled agent"
