...
def test_custom_non_spacedock_agent_id_falls_back_to_generic_worker():
    ...
    assert resolved["asset_path"].name == "codex-worker-prompt.md"
...
def test_packaged_worker_bootstrap_tells_worker_to_read_skill_first():
    resolved = resolve_codex_worker("spacedock:ensign")
    ...
    prompt = build_codex_worker_bootstrap_prompt(
        ...
        worktree_path=Path("/tmp/project/.worktrees/spacedock-ensign-entity"),
        checklist=["Write the stage report", "Commit code changes"],
    )

    assert "Read" in prompt
    assert str(resolved["asset_path"]) in prompt
